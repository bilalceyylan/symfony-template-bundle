...
class IconExtension extends AbstractExtension
{
    ...
    public function getIcon(string $identifier, string $size = 'small'): string
    {
        $identifier = $this->getAliasForIdentifier($identifier);
        $icon = $this->getIconForIdentifier($identifier);

        if ($icon) {
            $url = $this->packages->getUrl('bundles/template/icons/' . $icon['sprite'], 'template');
            $markup = [];
            $markup[] = '<span class="icon icon-size-' . $size . ' icon-state-default">';
            $markup[] =     '<span class="icon-markup">';
            $markup[] =         '<svg role="img"><use xlink:href="' . $url . '"></use></svg>';
            $markup[] =     '</span>';
            $markup[] = '</span>';
    ...
    {
        $iconDataCache = $this->cache->getItem('icon.data');
        if (!$iconDataCache->isHit()) {
            $iconDataJson = file_get_contents($this->kernel->locateResource('@TemplateBundle/Resources/public/icons/icons.json'));
